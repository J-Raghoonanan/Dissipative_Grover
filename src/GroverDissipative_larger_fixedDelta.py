...
def standard_continuous_reference_curve(
    params: LargeFigureParams,
    tau_ref: float,
    n_periods: float,
    points_per_period: int,
    ...
    """
    Standard continuous Grover baseline evaluated on a common reference time axis.

    This is used only for the blue r=0 curve so that the standard baseline has
    the same visual period in the r=4 and r=3 columns.  The dissipative and BJ
    curves still use each panel's own optimized Delta and tau.
    """
    times_ref = np.linspace(0.0, n_periods * tau_ref, int(n_periods * points_per_period) + 1)
    t_norm_ref = times_ref / tau_ref

    psi0_std = np.array([
        np.sqrt((params.N - params.M) / params.N),
        np.sqrt(params.M / params.N),
    ], dtype=complex)
    states_std = evolve_from_spectrum(build_reduced_standard_hamiltonian(params), psi0_std, times_ref)
    F_std = np.abs(states_std[:, 1]) ** 2
    return times_ref, t_norm_ref, F_std


def standard_discrete_reference_curve(
    params: LargeFigureParams,
    tau_ref: float,
    n_periods: float,
) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
    """
    Standard discrete Grover baseline evaluated on a common reference time axis.

    This is used only for the blue r=0 curve so that the standard baseline has
    the same visual period in the r=4 and r=3 columns.  The dissipative and BJ
    curves still use each panel's own optimized Delta and tau.
    """
    t_end_ref = n_periods * tau_ref
    n_steps = int(np.floor(t_end_ref / params.delta_t))
    step_times_ref = np.arange(n_steps + 1, dtype=float) * params.delta_t
    t_norm_ref = step_times_ref / tau_ref
    ...
    return step_times_ref, t_norm_ref, F_std
...
def plot_panel_continuous(
    ...
    points_per_period: int,
    label: str,
    standard_reference: Tuple[np.ndarray, np.ndarray, np.ndarray],
) -> List[Dict[str, Any]]:
    ...
    add_curve_rows(rows, metadata, "standard_common_reference", 0, times_std_ref, t_std_ref, F_std_ref)
    add_curve_rows(rows, metadata, "standard_panel_time_not_plotted", 0, times, t, F_std_panel)
...
def plot_panel_discrete(
    ...
    add_curve_rows(rows, metadata, "standard_common_reference", 0, step_times_std_ref, t_std_ref, F_std_ref)
    add_curve_rows(rows, metadata, "standard_panel_time_not_plotted", 0, step_times, t, F_std_panel)
...
def make_larger_system_figure(
    ...
    delta_t: float = np.pi,
    output_stem: str = "GroverDissipative_fig_larger_system_common_blue",
) -> Tuple[Path, Path]:
    ...
    # Use the r_top column as the common reference time unit for the blue
    # standard-Grover baseline.  This keeps the blue curve identical in the
    # r=4 and r=3 columns, while leaving the red dissipative and BJ curves
    # unchanged from the optimized-Delta calculation.
    tau_ref = bj_tau(params_r4.delta)
    std_cont_ref = standard_continuous_reference_curve(params_r4, tau_ref, n_periods, points_per_period)
    std_disc_ref = standard_discrete_reference_curve(params_r4, tau_ref, n_periods)

    rows: List[Dict[str, Any]] = []
    rows.extend(plot_panel_continuous(axes[0, 0], params_r4, n_periods, points_per_period, "(a)", std_cont_ref))
    rows.extend(plot_panel_continuous(axes[0, 1], params_r3, n_periods, points_per_period, "(b)", std_cont_ref))
    rows.extend(plot_panel_discrete(axes[1, 0], params_r4, n_periods, "(c)", std_disc_ref))
    rows.extend(plot_panel_discrete(axes[1, 1], params_r3, n_periods, "(d)", std_disc_ref))
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--output-stem", type=str, default="GroverDissipative_fig_larger_system_common_blue", help="Output filename stem.")
